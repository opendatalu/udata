...
# def log_request_headers(app):
#     @app.before_request
#     def before_request():
#         app.logger.info(f"Request Headers: {dict(request.headers)}")

#     @app.before_request
#     def log_raw_request_headers():
#         # Access raw headers from `environ` dictionary
#         raw_headers = {key: value for key, value in request.environ.items() if key.startswith('HTTP_')}
#         app.logger.info(f"Raw Request Headers: {raw_headers}")
...
def create_app(config='udata.settings.Defaults', override=None,
    ...
    app.executor = Executor(app)  # Added to trigger av scans [#61]

    init_logging(app)
    register_extensions(app)

    # if app.debug:
    #     log_request_headers(app)
