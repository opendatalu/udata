...
from udata.core.storages.tasks import run_antivirus_scan
...
DEFAULT_SORTING = "-last_modified_internal"
...
community_parser.add_argument(
    "sort",
    type=str,
    default="-last_modified_internal",
    location="args",
...
class UploadMixin(object):
    def handle_upload(self, dataset):
        ...
        # del infos["filename"]  # Not in mongo, but we need it for the antivirus scan
        return infos
...
class UploadNewDatasetResource(UploadMixin, API):
    ...
    def post(self, dataset):
        ...
        if resource.url.startswith("http"):
            full_resource_url = resource.url
        else:
            full_resource_url = "https://{host}/resources/{path}".format(
                host=current_app.config["FS_S3_SINGLE_BUCKET"],
                path=str(resource.url).lstrip("/"),
            )

        full_dataset_url = "https://{host}/fr/dataset/{path}".format(
            host=current_app.config["SERVER_NAME"], path=str(dataset.id)
        )

        run_antivirus_scan.delay(
            fs_filename=infos["fs_filename"],
            filename=filename,
            send_mail="info@data.public.lu",
            resource_id=str(resource.id),
            dataset_id=str(dataset.id),
            resource_url=full_resource_url,
...
class UploadNewCommunityResources(UploadMixin, API):
    ...
    def post(self, dataset):
        ...
        run_antivirus_scan.delay(
            ...
            filename=filename,
            send_mail="info@data.public.lu",
            resource_id=str(community_resource.id),
            dataset_id=str(dataset.id),
            resource_url=community_resource.url,
            dataset_url=full_dataset_url,
            send_if_ok=False,
        )
...
class ReuploadCommunityResource(ResourceMixin, UploadMixin, API):
    ...
    def post(self, community):
        ...
        try:
            dataset_url = "https://{host}/fr/dataset/{path}".format(
                host=current_app.config["SERVER_NAME"], path=str(community.dataset.id)
            )
        except:
            dataset_url = None
        ...
        run_antivirus_scan.delay(
            ...
            filename=filename,
            send_mail="info@data.public.lu",
            resource_id=str(community.id),
            dataset_id=str(community.dataset.id),
            resource_url=community.url,
