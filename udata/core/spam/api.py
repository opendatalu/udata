...
from udata.features.mcaptcha_validate import validate_captcha_token
...
mcaptcha_model = ns.model('McaptchaForm', {
    'mcaptcha_token': restx_fields.String(required=True, description='MCaptcha token')
})

@ns.route("/check")
class MCaptchaCheckAPI(API):
    """
    MCaptcha check API.
    """
    @ns.expect(mcaptcha_model)
    @api.doc('check_mcaptcha')
    def post(self, endpoint='check'):
        """
        Check the MCaptcha token from an app component.
        """
        mcaptcha_token = api.payload.get('mcaptcha_token')
        if validate_captcha_token(mcaptcha_token):
            return {}, 200
        return {}, 400
